'use client'


import React, { useEffect, useState } from "react";
import Container from "./Container";

const MemoryList = () => {

    const [memoryList, setMemoryList] = useState([]);
    const [limit, setLimit] = useState(3);
    const [page, setPage] = useState(0);
    const [pages, setPages] = useState(0);
  
    useEffect(() => {
      async function fetchMemoryList() {
        const rawMemoryList = await fetch(
          "https://ndc.energy.mn/wp-admin//api/memory-of-ahmads",{
            
            headers: {
             
              "Authorization": "Bearer "
            },
          }
        );
        
        const memoryList = await rawMemoryList.json();
        console.log("============", memoryList.data);
        const sortedMemoryListData = memoryList.data.sort((a, b) => a.attributes.id - b.attributes.id); // Ascending order
        console.log("memoryList", sortedMemoryListData);
  
        setMemoryList(sortedMemoryListData);
      }
      fetchMemoryList();
    }, [limit]);

    const changePage = ({ selected }) => {
      setPage(selected);
      if (selected === 9) {
        setMsg(
          "Jika tidak menemukan data yang Anda cari, silahkan cari data dengan kata kunci spesifik!"
        );
      } else {
        setMsg("");
      }
    };


    return (
        <Container>

            <h2 className="flex justify-center text-xl font-bold text-md uppercase text-text py-8"> Ахмадын дурсамж</h2>

      <div className="overflow-x-scroll overflow-hidden relative w-full text-left">
      <table id="tableEmpList" className="w-full table-auto border border-gray-400 divide-y  text-text overflow-y-scroll ">
        <thead className="font-bold uppercase text-left">
          <tr className="bg-gray-300 ">
                <th className="text-center">
                  №
                </th>
                <th>
                  Гарчиг
                </th>
                <th>
                  Файл
                </th>
                
          </tr>
        </thead>
        <tbody className="text-sm text-slate-600 divide-y ">
            {memoryList.map((item, index) => {
                      return (
                        <tr key={index}>
                          <th scope="row">{item.id}</th>
                          {/* <th scope="row " className="text-center">{item.attributes.emplooyee_id}</th> */}
                          <th scope="row">{item.attributes.title}</th>
                          <th scope="row">{item.attributes.filename}</th>
                          {/* <th scope="row">{item.attributes.major}</th>
                          <th scope="row">{item.attributes.phonenumber}</th>
                          <th scope="row">{item.attributes.mail}</th> */}
                        </tr>
                      );
                    })}
        </tbody>
        </table>
        
        </div>
        </Container>
    );
}
export default MemoryList;